-- ════════════════════════════════════════════════════════════════════════════
-- Daily auto-orders (subscriptions).
--   A customer subscribes to a set of items on a schedule (daily / chosen days)
--   with a delivery hour; every matching morning the app AUTO-CREATES that order
--   for them — same pricing, fees, stock checks and rewards as a normal order.
--
-- To reuse place_order's logic exactly (and never drift), its whole body is
-- lifted into _place_order_core(p_uid, …, p_enforce_store_open); place_order
-- becomes a one-line wrapper for the signed-in customer, and the subscription
-- engine calls the core on each subscriber's behalf.
-- ════════════════════════════════════════════════════════════════════════════

-- ── Core: place_order's body, parameterised by user + store-open enforcement ──
create or replace function public._place_order_core(
  p_uid uuid, p_items jsonb, p_coupon text DEFAULT NULL::text, p_location jsonb DEFAULT NULL::jsonb,
  p_payment text DEFAULT 'upi'::text, p_address text DEFAULT NULL::text, p_wallet numeric DEFAULT 0,
  p_redeem_points integer DEFAULT 0, p_membership boolean DEFAULT false,
  p_enforce_store_open boolean DEFAULT true)
...
$function$;
revoke execute on function public._place_order_core(uuid, jsonb, text, jsonb, text, text, numeric, integer, boolean, boolean) from public, anon, authenticated;

-- ── place_order becomes a thin wrapper for the signed-in customer ────────────
create or replace function public.place_order(
  p_items jsonb, p_coupon text DEFAULT NULL::text, p_location jsonb DEFAULT NULL::jsonb,
  p_payment text DEFAULT 'upi'::text, p_address text DEFAULT NULL::text, p_wallet numeric DEFAULT 0,
  p_redeem_points integer DEFAULT 0, p_membership boolean DEFAULT false)
 RETURNS public.orders
 LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
begin
  return public._place_order_core(auth.uid(), p_items, p_coupon, p_location,
...
create table if not exists public.subscriptions (
  ...
  items          jsonb not null,                 -- [{ "id": productId, "qty": n }]
  address        text,
  location       jsonb,                           -- { lat, lng, distanceKm } snapshot
  payment_method text not null default 'cod',     -- 'cod' | 'wallet'
  dow            int[],                           -- IST days 0=Sun..6=Sat; null = every day
  deliver_hour   int not null default 8,          -- IST hour the order is created
  active         boolean not null default true,
  skip_next      boolean not null default false,  -- skip the next occurrence only
  paused_until   date,                            -- skip while today < this date
  last_run       date,
  created_at     timestamptz not null default now(),
  updated_at     timestamptz not null default now()
);
create index if not exists subs_due_idx on public.subscriptions (deliver_hour) where active;
...
-- ── The engine: create each due subscription's order (hourly) ────────────────
...
begin
  for r in
    ...
    where s.active
      and coalesce(s.deliver_hour, 8) = v_hour
      and (s.last_run is null or s.last_run < v_today)
      and (s.dow is null or v_dow = any(s.dow))
      and (s.paused_until is null or v_today >= s.paused_until)
  loop
    -- Skip exactly one occurrence when asked.
    if r.skip_next then
      update public.subscriptions set skip_next = false, last_run = v_today, updated_at = now() where id = r.id;
      continue;
    end if;

    v_pay := coalesce(r.payment_method, 'cod');
    v_wal := case when v_pay = 'wallet' then 1000000 else 0 end;
    begin
      v_order := public._place_order_core(r.user_id, r.items, null, r.location, v_pay, r.address, v_wal, 0, false, false);
      insert into public.notifications (user_id, title, body) values
        (r.user_id, 'Aapka daily order aa gaya 🥛',
         'Order ' || v_order.human_code || ' place ho gaya — jaldi ghar par! 🛵');
      v_count := v_count + 1;
    exception when others then
      -- Wallet couldn't cover → fall back to cash on delivery.
      if v_pay = 'wallet' then
        begin
          v_order := public._place_order_core(r.user_id, r.items, null, r.location, 'cod', r.address, 0, 0, false, false);
          insert into public.notifications (user_id, title, body) values
            (r.user_id, 'Aapka daily order aa gaya 🥛',
             'Wallet kam tha — order ' || v_order.human_code || ' COD par place kiya.');
          v_count := v_count + 1;
        exception when others then
          insert into public.notifications (user_id, title, body) values
            (r.user_id, 'Daily order nahi ho paya ⚠️',
             'Aaj ka auto-order place nahi ho saka. App me apni subscription check karo.');
        end;
      else
        insert into public.notifications (user_id, title, body) values
          (r.user_id, 'Daily order nahi ho paya ⚠️',
    ...
    update public.subscriptions set last_run = v_today, updated_at = now() where id = r.id;
  end loop;
  return v_count;
end; $function$;
revoke execute on function public.run_subscriptions() from public, anon, authenticated;

-- Hourly; the function fires each subscription at its own deliver_hour (IST).
select cron.unschedule('subscriptions') where exists (select 1 from cron.job where jobname='subscriptions');
select cron.schedule('subscriptions', '20 * * * *', 'select public.run_subscriptions()');
