-- ════════════════════════════════════════════════════════════════════════════
-- Two member types: Normal (hasn't bought membership) and Prime (bought it).
-- Prime gets the HIGHEST honeymoon perk; a Normal member gets a fixed fraction
-- of it (default 55%). Set the Prime perk once + one "% of Prime" dial, and the
-- Normal tier is always derived from it — it can never drift out of the 50–60%
-- band you want. The profit cap still bounds every giveaway.
-- ════════════════════════════════════════════════════════════════════════════

-- Prime = the master (highest) perk; normalPct = what a Normal member gets of it.
update public.settings
   set rewards = jsonb_set(rewards, '{lifecycle,member}',
       '{"pointsBoost":2.5,"pointsFloor":1.3,"discPct":12,"discFloorPct":2,"discMax":60}'::jsonb)
 where id = 1;
update public.settings
   set rewards = jsonb_set(rewards, '{lifecycle,normalPct}',
       coalesce(rewards->'lifecycle'->'normalPct', to_jsonb(55)))
 where id = 1;
...
declare
  ...
  v_tier         jsonb;
  v_npct         numeric := 1;       -- Normal member = this fraction of Prime
  v_n            int;
  v_welcome_ord  int;
  v_taper_ord    int;
  v_boost        numeric := 1;
...
begin
  ...
  if v_life_on then
    ...
    -- Prime = the master perk; a Normal member gets normalPct% of it.
    v_tier := coalesce(v_life->'member', '{}'::jsonb);
    v_boost      := coalesce((v_tier->>'pointsBoost')::numeric, 2.5);
    v_floor      := coalesce((v_tier->>'pointsFloor')::numeric, 1.3);
    v_disc_pct   := coalesce((v_tier->>'discPct')::numeric, 12);
    v_disc_floor := coalesce((v_tier->>'discFloorPct')::numeric, 2);
    ...
    if not v_is_member then
      ...
      v_floor      := 1 + (v_floor - 1) * v_npct;
      v_disc_pct   := v_disc_pct * v_npct;
      v_disc_floor := v_disc_floor * v_npct;
  ...
  -- The Prime member's own membership fee funds their honeymoon: give it back as
  -- extra reward budget over their welcome window (tapers with v_frac). This is
  -- their prepaid ₹99 returning as perks, not a shop loss — and it's what makes a
  -- NEW Prime member clearly out-earn a Normal member.
