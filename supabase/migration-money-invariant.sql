-- Money invariant: no order is ever sold at a loss.
--
-- Found by supabase/tests/loss-sweep.sql, which places real orders across every
-- combination of cart shape x distance x member x coupon x points and measures the
-- true P&L. It reported 236 losing combinations out of 360, worst -Rs253.
--
--   A  delivery radius was enforced only in the app, so any crafted or stale
--      request placed an order at any distance. The fee bands are sized for the
--      radius while rider pay keeps climbing, so every out-of-area order lost.
--   B  points redemption was capped at 20% of cart value and never by margin.
--   C  a 'capped' coupon capped at MARGIN, so it could take 100% of it and leave
--      the picker and rider unfunded.
--   D  guaranteed coupons then stacked on top of B.
--
-- B, C and D are one bug: every giveaway had its own cap, measured against margin,
-- blind to the others and to what fulfilment costs. Margin is not the budget --
-- what is left AFTER the order pays for itself is. They now share one pot.
--
-- After: 0 unintended losses. Guaranteed coupons still absorb a loss because that
-- is what the owner chose, but never more than the coupon's face value.
