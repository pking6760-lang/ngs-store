-- NGS Prime redesign — free delivery for members on every order (near+mid).
-- Members get ₹0 delivery in the near + mid zone unconditionally (the
-- cart-can-fund test is dropped); the far zone keeps a reduced flat fee
-- (rewards.membership.farFee, default ₹25). Both the checkout engine
-- (_place_order_core) and the cart preview RPC (quote_delivery) are patched
-- identically so the shown fee always equals the charged fee.
...
begin
  ...
  -- NGS Prime: free delivery on EVERY order in the near + mid zone (no
  -- cart-can-fund test — the fee waiver is the perk we fund with the ₹99). The
  -- true far zone keeps a reduced flat fee so long rider pay never runs away.
  v_free_perk := v_is_member and v_dist_km < coalesce(v_ops.far_zone_km_2, 1e9);
  if v_qualify_total >= v_free_thresh and not v_has_thin then v_delivery := 0;
  elsif v_free_perk then v_delivery := 0;
  elsif v_is_member then v_delivery := least(v_band_fee, coalesce((v_rewards->'membership'->>'farFee')::numeric, 25));
...
begin
  ...
  -- NGS Prime: free delivery on every order in the near + mid zone (no
  -- cart-can-fund test); the true far zone keeps a reduced flat fee. Mirrors
  -- _place_order_core exactly so the cart preview equals checkout.
  if (v_qualify >= v_thresh and v_affordable)
     or (p_is_member and v_dist < coalesce(v_ops.far_zone_km_2, 1e9)) then v_fee := 0;
  elsif p_is_member then v_fee := least(v_band, coalesce((v_settings.rewards->'membership'->>'farFee')::numeric, 25));
