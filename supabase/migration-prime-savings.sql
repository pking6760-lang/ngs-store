-- ════════════════════════════════════════════════════════════════════════════
-- Prime savings meter — record what each member saved vs the normal price.
--
-- Adds orders.member_savings. place_order accumulates, per line,
-- (normal_price − charged_price) × qty (0 for non-members / thin-margin items),
-- and stores the total on the order. The app shows "You saved ₹X with Prime"
-- on the receipt and a monthly total on the membership panel. Pure surfacing of
-- savings that already happen — no margin cost.
-- ════════════════════════════════════════════════════════════════════════════

alter table public.orders add column if not exists member_savings numeric not null default 0;
