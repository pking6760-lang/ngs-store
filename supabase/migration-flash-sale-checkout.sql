-- Flash sale, checkout integration: _place_order_core charges an active
-- flash price FLAT (no member/bulk stacking, like a combo component), in both
-- the pricing pass and the order_items INSERT pass. Expiry is automatic —
-- once flash_ends_at passes the branch is skipped and normal pricing returns.
-- (Full function redefined; only the two flash branches were added.)
