-- ════════════════════════════════════════════════════════════════════════════
-- Only a NEW (first-time) Prime member starts at 100% perk. A renewal is NOT a
-- new member: it gets a smaller START level (a limited hike, default 30% of the
-- peak), and after its window ends it drops back to the lowest floor. Each state
-- has its own start level; all decay to a floor.
--     Normal        start = normalPct  (55%)   → floor normal (3%)
--     New Prime      start = 100%               → floor prime  (8%)
--     Renewed Prime  start = renewalPct (30%)   → floor prime  (8%)
-- ════════════════════════════════════════════════════════════════════════════

-- Renewal start level (the limited hike, < 100%).
update public.settings
   set rewards = jsonb_set(rewards, '{lifecycle,renewalPct}',
       coalesce(rewards->'lifecycle'->'renewalPct', to_jsonb(30)))
 where id = 1;
...
declare
  ...
  v_start_frac   numeric := 1;       -- welcome level (fraction of the 100% peak)
  v_floor_frac   numeric := 0;       -- settled floor (fraction of the peak)
  v_frac         numeric := 1;       -- raw taper progress 0→1
  v_perk         numeric := 1;       -- perk level, start_frac → floor_frac
  v_mult         numeric := 1;
...
begin
  ...
  if v_life_on then
    ...
    -- Start (welcome) level + settled floor. ONLY a first-time Prime member = 100%.
    if not v_is_member then
      ...
      v_start_frac := coalesce((v_life->>'renewalPct')::numeric, 30) / 100.0;   -- renewal hike (< 100%)
      v_floor_frac := coalesce((v_life->'floorPct'->>'prime')::numeric, 8) / 100.0;
    else
      v_start_frac := 1.0;                                                       -- NEW Prime = 100%
      v_floor_frac := coalesce((v_life->'floorPct'->>'prime')::numeric, 8) / 100.0;
    end if;
    v_disc_max := round(v_disc_max * v_start_frac);   -- ₹ cap scales with the tier's peak
    ...
    v_perk     := v_start_frac - (v_start_frac - v_floor_frac) * v_frac;   -- start% → floor%
  ...
  -- Rewards come only from the order's own profit — no free money / welcome fund.
  -- The perk (bigger product discount + more points + better scratch) is bounded
  -- so the shop always keeps its floor.
