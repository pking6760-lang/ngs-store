-- Handling charge: ₹12, charged to every order — normal AND Prime customers.
-- Prime's perk is now free delivery only; handling always applies (it funds
-- the picker/packing cost, which exists on every order regardless of tier).

update public.ops_config set handling_fee = 12 where id = 1;  -- mirrors to settings
