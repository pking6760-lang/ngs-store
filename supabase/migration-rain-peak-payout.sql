-- Rain and peak: two surcharges, two different people, one flat charge.
--
-- They were one 'surge' mode and its whole bonus went to the RIDER, so a
-- peak-hour surcharge paid the driver for the packer's work. A busy hour is
-- packing pressure, not weather.
--
--   RAIN -> rider bonus    PEAK -> picker bonus    both -> both get theirs
--
-- The customer pays ONE flat surcharge (Rs25) whichever is on and however many.
-- delivery_mode is now normal | rain | peak | both; 'surge' still reads as rain
-- so nothing breaks on a shift already running. orders.surge_mode records which
-- applied, so admin payout estimates stop guessing from the fee amount.
--
-- Verified end to end against the wallet ledger:
--   normal  customer Rs0    picker 6.75   rider 23.00
--   rain    customer Rs25   picker 6.75   rider 35.00
--   peak    customer Rs25   picker 18.75  rider 23.00
--   both    customer Rs25   picker 18.75  rider 35.00
--
-- Note on 'both': Rs25 in, Rs24 of bonuses out. The surcharge is there to keep
-- people working in bad conditions, not to earn -- but that is the real margin.
...
begin
  ...
  for v_line in select * from jsonb_array_elements(p_items) loop
    ...
    -- Only the owner's MANUAL exclusion keeps an item out of the bar. The
    -- automatic margin flag used to do this too, but a per-item flag cannot
    -- answer a per-cart question: it can't see quantity. One bottle of oil
    -- can't fund a ride, ten can. The affordability test below handles it.
  ...
  v_min_free_profit := coalesce(v_ops.min_free_delivery_profit, 8);
...
CREATE OR REPLACE FUNCTION public.quote_delivery(p_items jsonb, p_distance_km numeric DEFAULT 0, p_is_member boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
...
declare
  ...
  v_is_rain boolean := false; v_is_peak boolean := false;
begin
  select * into v_ops from public.ops_config where id = 1;
  select * into v_settings from public.settings where id = 1;
  ...
  v_picker := case when v_ops.coverage_picking = 'staff' then
      round(coalesce(v_ops.picker_pack_fee,0) + v_lines * coalesce(v_ops.picker_per_line,0)
            + v_units * coalesce(v_ops.picker_per_unit,0)
            + case when v_is_peak then coalesce(v_ops.peak_bonus,0) else 0 end, 2) else 0 end;
  v_rider := case when v_ops.coverage_delivery = 'staff' then
      greatest(coalesce(v_ops.rider_base,0) + v_dist * coalesce(v_ops.rider_per_km,0),
               coalesce(v_ops.rider_min,0))
      + case when v_is_rain then coalesce(v_ops.rain_bonus,0) else 0 end else 0 end;
...
begin
  ...
  if v_is_milk then
    ...
    v_earn := round(
      ...
      + case when (select delivery_mode from public.settings where id = 1)
                    in ('rain', 'both', 'surge')
             then coalesce(cfg.rain_bonus, 0) else 0 end, 2);
  end if;
...
begin
  ...
  v_earn := round(coalesce(cfg.picker_pack_fee, 0)
                 ...
                 + coalesce(v_units, 0) * coalesce(cfg.picker_per_unit, 0)
            -- Peak pays the PICKER: a busy hour is packing pressure, not weather.
            + case when (select delivery_mode from public.settings where id = 1)
                        in ('peak', 'both')
