-- Prime members must still pay delivery+handling on thin-margin (milk/dairy)
-- orders, otherwise the waived fees leave the driver cost uncovered = loss.
...
select 'member milk fee leak fixed' as status;
