-- place_order: price each line with the product's bulk tiers (server-side, so
-- the per-unit break can't be spoofed by the client). Same logic the customer
-- app uses for display, kept authoritative here.
