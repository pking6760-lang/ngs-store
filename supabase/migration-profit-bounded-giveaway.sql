-- ════════════════════════════════════════════════════════════════════════════
-- Fund the new-customer generosity from REAL per-order profit, keep a floor.
--
-- Owner enters only buying price (+ MRP); the system auto-sets the selling price.
-- For each order we compute the true profit:
--     item margin (selling − cost) + fees charged − picker pay − driver pay
-- Then the new-customer giveaway (welcome discount + the boosted points/wallet)
-- is capped so the shop ALWAYS keeps a minimum profit — it never hits zero.
-- The invisible points/wallet boost is trimmed first; the visible discount only
-- as a last resort (thin-margin baskets). Floors are admin-tunable.
-- ════════════════════════════════════════════════════════════════════════════

-- Add the shop-profit floor to the lifecycle config.
update public.settings
   set rewards = jsonb_set(
     rewards,
     '{lifecycle}',
     coalesce(rewards->'lifecycle','{}'::jsonb)
       || jsonb_build_object(
            'shopFloorRupees', coalesce(rewards->'lifecycle'->'shopFloorRupees', to_jsonb(6)),
            'shopFloorPct',    coalesce(rewards->'lifecycle'->'shopFloorPct',    to_jsonb(3)))
   )
 where id = 1;
...
declare
  ...
  v_item_margin  numeric := 0;        -- true goods margin (selling − cost)
  v_cat_totals   jsonb := '{}';
  ...
  -- Boost split (base vs extra) + profit cap
  v_base_pts     int := 0;
  v_boost_pts    int := 0;
  v_extra_pts    int := 0;
...
begin
  ...
  -- Lifecycle factors for this (v_n-th) order.
  ...
  for v_line in select * from jsonb_array_elements(p_items) loop
    ...
    -- True goods margin (real cost when known; else the default-margin proxy).
  ...
  -- Fees (needed for the profit calc below).
  ...
  -- Desired welcome discount (tapers with order count), capped in ₹ / by what's left.
  ...
  -- Base (unboosted) vs boosted points/wallet — the boost is the giveaway.
  ...
  -- Real per-order profit, and the giveaway cap that protects the shop floor.
  ...
  -- Budget for the boost = profit left after the shop floor AND the (fixed,
  -- client-honest) welcome discount. The welcome discount is a small owner-set
  -- acquisition cost and is NOT trimmed here — trimming it would break the price
  -- shown at checkout, since the client can't see cost. The profit-bounded part
  -- is the invisible points/wallet boost, which we scale down to fit the budget.
  ...
  -- Finalize points + wallet (base + surviving boost), then the scratch split.
