-- ════════════════════════════════════════════════════════════════════════════
-- "Thin margin" product flag — opt a staple OUT of the reward economy.
--
-- New column products.no_rewards. When true, the place_order loop:
--   • charges the NORMAL price (bulk_unit_price), never the member tier price;
--   • adds nothing to the points pool (v_reward_margin);
--   • adds nothing to the scratch/high-margin pool (v_high_margin).
-- So the item gives NO member discount, NO points, NO scratch reward. It stays
-- fully buyable and still counts toward the bill. Independent of
-- free_delivery_exempt. The client mirrors this (tierUnitPrice in bulk.js).
--
-- Verified live: member charged normal 142 (vs deep-member 129); a normal order
-- that earned 15 points earned 0 with the flag on.
-- ════════════════════════════════════════════════════════════════════════════

alter table public.products add column if not exists no_rewards boolean not null default false;
