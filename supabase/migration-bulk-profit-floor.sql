-- ════════════════════════════════════════════════════════════════════════════
-- Bulk pricing fix: a bigger pack must NEVER earn the shop less total profit
-- than a smaller one. The old rule only floored each unit at cost+markup, so a
-- thin per-unit margin on a big pack could total LESS than a single unit
-- (e.g. pack-of-4 profit < pack-of-1). Now each tier's price is floored so the
-- WHOLE pack earns at least as much as the previous tier — while still giving a
-- real per-unit discount for buying more.
-- ════════════════════════════════════════════════════════════════════════════
drop function if exists public.build_bulk_tiers(numeric, numeric, public.pricing_config);
...
declare
  ...
  prev_profit numeric := base - coalesce(cost, 0);   -- total profit on a single unit
...
begin
  ...
  for i in 1..3 loop
    ...
    -- Per-unit price = the configured discount, but never below the cost floor,
    -- and never so low that the whole pack (q units) earns less total profit
    -- than the previous tier. That guarantees: buy more → shop earns at least
    -- as much (never less), and the customer still pays less per unit.
    p := greatest(
           round(base * (1 - coalesce(off, 0))),
           floor_price,
           ceil(c + prev_profit / q)
         );
    if p >= last then continue; end if;         -- no genuine per-unit discount left → skip tier
...
-- Re-run the repricer so live products pick up the corrected tiers.
