-- ============================================================================
-- Wallet double-spend / race hardening for place_order.
--
-- Two fixes (captured from the live function, applied verbatim otherwise):
--   1. Debit the NGS wallet at PLACEMENT for ONLINE orders too (previously the
--      debit was deferred to mark_order_paid). Deferring let a customer stack
--      several unpaid online orders each claiming the full wallet balance, then
--      pay them all → the wallet went negative (spent > balance). Debiting at
--      placement drops the balance immediately, so later orders see the reduced
--      balance. Abandoned online orders are auto-refunded by the existing
--      wallet_restore_on_cancel trigger (a 'spent' row now exists), and
--      mark_order_paid already skips the debit when a 'spent' row is present.
--   2. Take a per-user transaction advisory lock before reading the wallet
--      balance, so two truly-concurrent orders can't both spend the same money.
-- ============================================================================
...
begin
  ...
  for v_line in select * from jsonb_array_elements(p_items) loop
    ...
    -- Normal (non-member) unit price for this line — the reference we discount from.
    v_bulk := public.bulk_unit_price(v_prod.price, v_prod.bulk_tiers, v_qty);
    -- Thin-margin staples opt out of member pricing: everyone pays normal price.
    ...
    -- Prime savings meter: what this member saved vs the normal price (0 otherwise).
    ...
    -- Thin-margin staples earn NO points and NO scratch/high-margin reward.
  ...
  -- Referral reward: on the referee's FIRST order, pay both sides to wallet.
  -- The referrer's payout is capped by this order's profit so the shop never
  -- pays out more than it earned. Store credit only — earned by a real purchase.
  if not v_online and coalesce(v_profile.order_count, 0) = 0 then
    declare v_rf public.referrals; v_amt numeric; v_cap numeric;
    begin
      select * into v_rf from public.referrals where referee_id = v_uid and status = 'pending';
      if v_rf.id is not null then
        v_amt := coalesce(v_rf.reward_amount, 30);
        v_cap := greatest(least(v_amt, floor(greatest(v_profit, 0))), 0);
        insert into public.customer_wallet (user_id, amount, kind, note, order_id, created_by)
          values (v_uid, v_amt, 'referral', 'Referral welcome reward', v_order.id, v_uid);
        if v_cap > 0 then
          insert into public.customer_wallet (user_id, amount, kind, note, order_id, created_by)
