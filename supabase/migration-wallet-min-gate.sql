-- Referral-farming guards (customer wallet + delivery pricing).
--
--  #1  Wallet minimum-order gate: the ₹30 referral/welcome credit (and any other
--      wallet balance) can only be redeemed on a cart of ₹199+ (configurable via
--      rewards.walletMinOrder). Previously wallet money came straight off ANY
--      order total, so a ₹30 credit could be burned on a tiny order that doesn't
--      even cover the rider — a guaranteed loss and an easy farm.
--
--  #2  Delivery pricing: free delivery only above ₹199; below that the customer
--      pays a ₹20 delivery fee. A ₹10 handling fee applies on all orders (members
--      keep their free-delivery perk). Set on the settings row below.
--
-- Both values are editable later from Admin → Delivery without another migration.

-- config: the ₹199 free-delivery / fee structure. Charges are single-sourced in
-- ops_config; a trigger mirrors them into public.settings for the customer app,
-- so we set them here (setting settings.* directly would be overwritten).
...
-- wallet-redemption floor lives in the rewards blob (not mirrored from ops_config).
update public.settings
   set rewards = jsonb_set(rewards, '{walletMinOrder}', '199'::jsonb, true)
 where id = 1;

-- function: enforce the wallet floor server-side (authoritative) ---------------
