-- ════════════════════════════════════════════════════════════════════════════
-- Referral hardening (audit round 2): lock the referral row against a
-- double-reward race, and cap the referee's welcome reward by the order's
-- profit too (previously uncapped) so the shop never pays out more than it
-- earned. Full current place_order body captured from the live DB.
-- ════════════════════════════════════════════════════════════════════════════
  ...
  if p_coupon is not null and length(trim(p_coupon)) > 0 then
    ...
    if v_coupon.code is not null then
      ...
      if v_eligible >= v_coupon.min_order and v_eligible > 0 then
        ...
        -- Loss guard: a coupon can never eat past the product margin (never below cost).
