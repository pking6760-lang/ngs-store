-- Giveaways stop at the shop's floor, not at break-even.
--
-- Found by running every scenario against a real order (NGS4080, Rs440):
-- EVERY scenario where the customer redeemed points landed on exactly Rs0.92
-- profit. Not a loss, so the invariant held -- but points were consuming 100%
-- of the upside on every order. An order that clears Rs0 has paid the rider and
-- the picker and left the shop nothing for stock, rent or shrink.
--
-- Points are the one giveaway a customer triggers unprompted on every order, so
-- they were the one that always drained it. The budget now reserves the existing
-- shop floor (Rs6 or 3% of the cart, whichever is larger) before anything is
-- given away -- the same floor the scratch reward already respected.
--
-- On NGS4080: points scenarios go Rs0.92 -> Rs13.92. Across the full sweep the
-- worst in-area case goes Rs0.00 -> Rs6.36, still 0 unintended losses.
-- Guaranteed coupons are unaffected: they exist to go below the line.
