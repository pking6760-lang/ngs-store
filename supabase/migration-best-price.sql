-- ════════════════════════════════════════════════════════════════════════════
-- Restore the real (tier-pricing) smart_reprice — an earlier migration copied an
-- OLD body that referenced products.cost (now in product_costs), which broke it.
-- Rebuild the correct function and fold in two changes:
--   1. manual_price protection — never auto-reprice a hand-set price.
--   2. "Best Prices" is DISCOUNT-driven: any active product with a real discount
--      off MRP (>= 5%) is featured, plus pinned, plus fast-sellers, minus hidden.
-- ════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  update public.products p set
    -- Keep the price for 'unpriced' items AND anything priced by hand.
  ...
  -- Bulk tiers are (re)built from the (possibly hand-set) selling price.
  update public.products p set
  ...
  -- Real discount off MRP (>= 5%) → featured. This is what the owner discounts.
  update public.products p set bait = true
    from public.product_costs pc
  ...
  -- Tier pricing anchor: the deepest price any member can ever pay.
...
-- Recompute now so hand-discounted products appear in Best Prices immediately.
