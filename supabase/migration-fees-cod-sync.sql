-- ════════════════════════════════════════════════════════════════════════════
-- Task #7 — Customer checkout: COD limit + surge/fees single-source-of-truth.
--
-- Problem: the owner's charge dials lived in BOTH `ops_config` (partner engine)
-- and `settings` (what the customer app and place_order actually read), edited
-- from two different admin screens. They drifted. Also the COD cash cap
-- (ops_config.cod_customer_limit) was configured but never enforced.
--
-- Fix:
--   1. `ops_config` is the single source of truth for the 5 customer charges.
--      A trigger mirrors them into `settings` on every change, so the customer
--      app + place_order (which read `settings`) are always in step.
--   2. place_order rejects a Cash-on-Delivery order whose grand total exceeds
--      the COD limit — the rider never has to carry more cash than allowed.
-- ════════════════════════════════════════════════════════════════════════════

-- Mirror target for the COD cap on the customer-facing settings row.
alter table public.settings
  add column if not exists cod_customer_limit numeric(10,2) not null default 1000;

-- ── Mirror ops_config → settings ────────────────────────────────────────────
...
drop trigger if exists ops_config_sync on public.ops_config;
create trigger ops_config_sync
  after insert or update on public.ops_config
  for each row execute function public.sync_ops_to_settings();

-- One-time initial sync so the two tables start consistent.
update public.settings s set
  handling_fee        = o.handling_fee,
  delivery_fee        = o.delivery_fee,
  free_delivery_above = o.free_delivery_threshold,
  surge_fee           = o.surge_fee,
  cod_customer_limit  = o.cod_customer_limit
from public.ops_config o
where s.id = 1 and o.id = 1;

-- ── place_order: everything from the free-delivery-exempt version, plus the
--    COD-limit guard. Free delivery still uses the exempt-aware qualifying total.
...
begin
  ...
  -- Cash-on-delivery cap: keep a rider from carrying more cash than allowed.
